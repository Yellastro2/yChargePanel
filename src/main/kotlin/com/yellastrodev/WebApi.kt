...
//                    call.respond(HttpStatusCode.OK, """{"status": "client not connected"}""")
            ...
            get("/$ROUT_STATIONINFO") {
                try {
                    ...
                    val fStationJson = JSONObject().apply {
                        ...
                        put(KEY_STATUS, fStation.status)
            ...
            get("/$ROUT_DISABLE_STATION") {
                ...
                fStation?.let {
                    it.status = if (it.status == Station.Status.BLOCKED) Station.Status.AVAILABLE else Station.Status.BLOCKED
                    database.updateStation(fStation)
                    call.respond(HttpStatusCode.OK, """{"status": "$it.status"}""")
                } ?: run {
                    call.respond(HttpStatusCode.NotFound, """{"status": "station not found"}""")
                }
